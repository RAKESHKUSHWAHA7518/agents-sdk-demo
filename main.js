...
let auth = { type: 'key', clientKey: "Z29vZ2xlLW9hdXRoMnwxMDEwMjk3NTAyMTY2MTQwNzcwODc6U2VoLXVhaW93SFFGUTBCTzZRVGZa" };
...
const callbacks = {
    ...
    onConnectionStateChange(state) {
        ...
        if (state == "connecting") {
            ...
            document.querySelector("#agent-left").style.display = "flex"
            ...
            // document.querySelector("#agent-left").style.display = "none"
...
async function speak() {
    if (textArea.value !== "" && textArea.value.length > 2) {
        // Stop the other agent before starting
       
        await agentManager.speak({ type: "text", input: textArea.value });
        textArea.value = "";

         if (agentManager2) {
            await agentManager2.disconnect();
            console.log("Connecting to Agent ID: ", agentId2)
await agentManager2.connect()

// Check for the set Stream type (Legacy/Fluent)
streamType2 = agentManager2.getStreamType()
console.log("Stream Type:", streamType2) // optional chaining if supported
        }
    }
}
...
async function chat() {
    if (textArea.value !== "") {
        ...
        //     await agentManager2.disconnect();
        // }
        await agentManager.chat(textArea.value);
        connectionLabel.innerHTML = "Thinking..";
        textArea.value = "";
         if (agentManager2) {
            await agentManager2.disconnect();
...
function stopAgent() {
    if (agentManager) {
        ...
        connectionLabel.innerHTML = "Stopped";
        document.querySelector("#hidden_h2").innerHTML = `${agentManager.agent.preview_name} Stopped`;
        restartAgent();
        // document.querySelector("#hidden").style.display = "flex";
        // document.querySelector("#agent-left").style.display = "none";
        // actionButton.setAttribute("disabled", true);
...
let auth2 = { type: 'key', clientKey: "Z29vZ2xlLW9hdXRoMnwxMDEwMjk3NTAyMTY2MTQwNzcwODc6U2VoLXVhaW93SFFGUTBCTzZRVGZa" };
...
let streamVideoElement2 = document.querySelector("#streamVideoElement2")
let idleVideoElement2 = document.querySelector("#idleVideoElement2")
let textArea2 = document.querySelector("#textArea2")
let speechButton2 = document.querySelector("#speechButton2");
let answers2 = document.querySelector("#answers2")
let connectionLabel2 = document.querySelector("#connectionLabel2")
let reconnectButton2 = document.querySelector("#reconnectButton2")
let actionButton2 = document.querySelector("#actionButton2");
let videoWrapper2 = document.querySelector("#video-wrapper2");
let srcObject2;
let streamType2;
...
const callbacks2 = {
    ...
    onSrcObjectReady(value) {
        console.log("SrcObject Ready")
        streamVideoElement2.srcObject = value
        srcObject2 = value
        return srcObject2
    },
    ...
    onConnectionStateChange(state) {
        ...
        if (state == "connecting") {
            connectionLabel2.innerHTML = "Connecting.."
            document.querySelector("#agent-right").style.display = "flex"
            document.querySelector("#hidden").style.display = "none"

            // Displaying the Agent's name in the pages' header
            document.querySelector("#previewName2").innerHTML = agentManager2.agent.preview_name

            // Setting the video elements' sources and display
            idleVideoElement2.src = agentManager2.agent.presenter.idle_video
            idleVideoElement2.play()
            videoWrapper2.style.filter = "blur(5px)"
            streamVideoElement2.style.opacity = 0;
            idleVideoElement2.style.opacity = 1;

            // Setting video background image to avoid "flickering" for the legacy streaming architecture
            videoWrapper2.style.backgroundImage = `url(${agentManager2.agent.presenter.thumbnail})`
            // For photo-based avatars, set the following: `url(${agentManager.agent.presenter.source_url})`
            // For Premium+ avatars, set the following: `url(${agentManager.agent.presenter.thumbnail})`
            // Alternativley, save the first frame of the Idle video locally and set it as background image.
        }
            ...
            // Setting the 'Tab' key to switch between the modes and 'Enter' Key to Send a message
            textArea2.addEventListener('keypress', (event) => { if (event.key === "Enter") { event.preventDefault(); handleAction2() } })
            document.addEventListener('keydown', (event) => { if (event.key === 'Tab') { event.preventDefault(); switchModes2() } })
            actionButton2.removeAttribute("disabled")
            speechButton2.removeAttribute("disabled")
            answers2.innerHTML += `<div class='agentMessage'>${agentManager2.agent.greetings[0]}</div><br>`
            if (streamType !== StreamType.Fluent) {
                connectionLabel2.innerHTML = "Connected"
                videoWrapper2.style.filter = "blur(0px)"
            }
        }

        else if (state == "disconnected" || state == "closed") {
            textArea2.removeEventListener('keypress', (event) => { if (event.key === "Enter") { event.preventDefault(); handleAction2() } })
            document.removeEventListener('keydown', (event) => { if (event.key === 'Tab') { event.preventDefault(); switchModes2() } })
            document.querySelector("#hidden_h2").innerHTML = `${agentManager2.agent.preview_name} Disconnected`
            // document.querySelector("#hidden").style.display = "flex"
            // document.querySelector("#agent-right").style.display = "none"
            // actionButton2.setAttribute("disabled", true)
            // speechButton2.setAttribute("disabled", true)
            // document.getElementById("video-wrapper2").style.filter = "blur(5px)"
            // connectionLabel2.innerHTML = ""
        }
    },
    ...
    onVideoStateChange(state) {
        console.log("Video State: ", state)
        // NEW ARCHITECURE (Fluent: Single Video for both Idle and Streaming)
        if (streamType2 == StreamType.Fluent) {
            if (state == "START") {
                videoWrapper2.style.filter = "blur(0px)"
                connectionLabel2.innerHTML = "Connected"
                streamVideoElement2.style.opacity = 1;
                idleVideoElement2.style.opacity = 0;
            }
        }
        // OLD ARCHITECURE (Legacy: Switching between the idle and streamed videos elements)
        else {
            if (state == "START") {
                streamVideoElement2.muted = false
                streamVideoElement2.srcObject = srcObject2
                idleVideoElement2.style.opacity = 0;
                streamVideoElement2.style.opacity = 1;
            }
            else {
                streamVideoElement2.muted = true
                idleVideoElement2.style.opacity = 1;
                streamVideoElement2.style.opacity = 0;
            }
        }
    ...
    onNewMessage(messages, type) {
        ...
        if (msg && msg.role == "assistant" && messages.length != 1) {
            if (type == "answer") {
                answers2.innerHTML += `<div class='agentMessage'> ${msg.content} <div class="ratingButtons"> <button id='${msg.id}_plus' title='agentManager.rate2() -> Rate this answer (+)'><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="none">
                <path fill="currentcolor" d="m11.986 7.1-.41-.071.41.07Zm-.392 2.266-.41-.07.41.07Zm-7.406 2.34-.415.036.415-.036Zm-.452-5.218.416-.036-.416.036Zm4.372-3.257.411.067-.41-.067ZM7.74 5.478l.411.067-.41-.067Zm-3.685.229-.272-.316.272.316Zm.799-.69.272.317-.272-.316Zm1.323-2.03-.404-.104.404.105ZM6.44 1.97l.403.105-.403-.105Zm.93-.492-.128.397.128-.397Zm.08.026.128-.397-.128.397ZM5.812 3.921l.368.196-.368-.196Zm2.246-1.787-.403.105.403-.105ZM6.82 1.515l-.18-.375.18.375Zm-4.28 10.743-.414.036.415-.036ZM2 6.016l.415-.036a.417.417 0 0 0-.832.036H2Zm9.576 1.013-.392 2.266.82.142.393-2.266-.821-.142Zm-3.884 5.107H5.109v.833h2.583v-.833Zm-3.09-.465-.45-5.219-.83.072.45 5.218.83-.071Zm6.582-2.376c-.282 1.629-1.75 2.84-3.492 2.84v.834c2.125 0 3.958-1.483 4.313-3.532l-.821-.142ZM7.697 3.164 7.33 5.41l.822.134.368-2.247-.822-.134Zm-3.37 2.858.799-.688-.544-.632-.8.689.545.631Zm2.253-2.93.264-1.018-.806-.21-.265 1.02.807.209Zm.663-1.218.081.026.255-.794-.08-.026-.256.794ZM6.18 4.117c.173-.325.308-.668.4-1.024l-.807-.21a3.821 3.821 0 0 1-.328.841l.735.393ZM7.324 1.9c.17.054.291.186.33.34l.807-.21a1.325 1.325 0 0 0-.882-.924l-.255.794Zm-.48.174A.286.286 0 0 1 7 1.89l-.362-.75a1.119 1.119 0 0 0-.6.725l.806.209ZM7 1.89a.331.331 0 0 1 .243-.016l.255-.794a1.164 1.164 0 0 0-.86.06L7 1.89Zm1.196 4.543h2.879v-.834H8.196v.834Zm-5.241 5.79-.54-6.243-.83.072.54 6.242.83-.071Zm-.538.059V6.016h-.834v6.266h.834Zm-.292.012a.146.146 0 0 1 .145-.158v.833c.404 0 .72-.345.685-.746l-.83.071Zm6.394-8.996c.07-.422.05-.854-.058-1.268l-.806.21c.078.301.093.616.042.924l.822.134Zm-3.41 8.838a.509.509 0 0 1-.506-.465l-.83.071c.06.694.64 1.227 1.336 1.227v-.833Zm.017-6.802c.378-.326.785-.713 1.054-1.217l-.735-.393c-.193.36-.5.665-.863.978l.544.632Zm7.27 1.837a1.342 1.342 0 0 0-1.321-1.572v.834c.315 0 .555.284.5.596l.822.142ZM2.27 12.136c.082 0 .147.066.147.146h-.834c0 .379.307.687.687.687v-.833ZM7.33 5.41a.88.88 0 0 0 .867 1.022v-.834a.046.046 0 0 1-.045-.054l-.822-.134ZM4.152 6.452a.51.51 0 0 1 .175-.43l-.544-.631a1.343 1.343 0 0 0-.462 1.133l.83-.072Z"/>
                </svg></button> <button id='${msg.id}_minus' title='agentManager.rate2() -> Rate this answer (-)'><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="none" style="rotate: 180deg;">
                <path fill="currentcolor" d="m11.986 7.1-.41-.071.41.07Zm-.392 2.266-.41-.07.41.07Zm-7.406 2.34-.415.036.415-.036Zm-.452-5.218.416-.036-.416.036Zm4.372-3.257.411.067-.41-.067ZM7.74 5.478l.411.067-.41-.067Zm-3.685.229-.272-.316.272.316Zm.799-.69.272.317-.272-.316Zm1.323-2.03-.404-.104.404.105ZM6.44 1.97l.403.105-.403-.105Zm.93-.492-.128.397.128-.397Zm.08.026.128-.397-.128.397ZM5.812 3.921l.368.196-.368-.196Zm2.246-1.787-.403.105.403-.105ZM6.82 1.515l-.18-.375.18.375Zm-4.28 10.743-.414.036.415-.036ZM2 6.016l.415-.036a.417.417 0 0 0-.832.036H2Zm9.576 1.013-.392 2.266.82.142.393-2.266-.821-.142Zm-3.884 5.107H5.109v.833h2.583v-.833Zm-3.09-.465-.45-5.219-.83.072.45 5.218.83-.071Zm6.582-2.376c-.282 1.629-1.75 2.84-3.492 2.84v.834c2.125 0 3.958-1.483 4.313-3.532l-.821-.142ZM7.697 3.164 7.33 5.41l.822.134.368-2.247-.822-.134Zm-3.37 2.858.799-.688-.544-.632-.8.689.545.631Zm2.253-2.93.264-1.018-.806-.21-.265 1.02.807.209Zm.663-1.218.081.026.255-.794-.08-.026-.256.794ZM6.18 4.117c.173-.325.308-.668.4-1.024l-.807-.21a3.821 3.821 0 0 1-.328.841l.735.393ZM7.324 1.9c.17.054.291.186.33.34l.807-.21a1.325 1.325 0 0 0-.882-.924l-.255.794Zm-.48.174A.286.286 0 0 1 7 1.89l-.362-.75a1.119 1.119 0 0 0-.6.725l.806.209ZM7 1.89a.331.331 0 0 1 .243-.016l.255-.794a1.164 1.164 0 0 0-.86.06L7 1.89Zm1.196 4.543h2.879v-.834H8.196v.834Zm-5.241 5.79-.54-6.243-.83.072.54 6.242.83-.071Zm-.538.059V6.016h-.834v6.266h.834Zm-.292.012a.146.146 0 0 1 .145-.158v.833c.404 0 .72-.345.685-.746l-.83.071Zm6.394-8.996c.07-.422.05-.854-.058-1.268l-.806.21c.078.301.093.616.042.924l.822.134Zm-3.41 8.838a.509.509 0 0 1-.506-.465l-.83.071c.06.694.64 1.227 1.336 1.227v-.833Zm.017-6.802c.378-.326.785-.713 1.054-1.217l-.735-.393c-.193.36-.5.665-.863.978l.544.632Zm7.27 1.837a1.342 1.342 0 0 0-1.321-1.572v.834c.315 0 .555.284.5.596l.822.142ZM2.27 12.136c.082 0 .147.066.147.146h-.834c0 .379.307.687.687.687v-.833ZM7.33 5.41a.88.88 0 0 0 .867 1.022v-.834a.046.046 0 0 1-.045-.054l-.822-.134ZM4.152 6.452a.51.51 0 0 1 .175-.43l-.544-.631a1.343 1.343 0 0 0-.462 1.133l.83-.072Z"/>
                </svg></button></div>
                </div><br> `
                console.log(`New Message:\n[${msg.role}] ${msg.content}`)
                connectionLabel2.innerHTML = "Online"
                document.getElementById(`${msg.id}_plus`).addEventListener('click', () => rate2(msg.id, 1))
                document.getElementById(`${msg.id}_minus`).addEventListener('click', () => rate2(msg.id, -1))
            }

            // User Messages
        } else if (!!msg) {
            answers2.innerHTML += `<div class="userMessage">${msg.content} </div><br>`
            console.log(`New Message:\n[${msg.role}] ${msg.content}`)
        }

        // Auto-scroll to the last message 
        answers2.scrollTo({
            top: answers2.scrollHeight + 50,
            behavior: 'smooth'
        });
    ...
    // Error handling
    onError(error, errorData) {
        connectionLabel2.innerHTML = `<span style="color:red">Something went wrong :(</span>`
        console.log("Error:", error, "Error Data:", errorData)
    }
...
// 5. Define the Stream options object
let streamOptions2 = { compatibilityMode: "on", streamWarmup: false, fluent: false }
...
//     // 'Speak' supports a minimum of 3 characters
//     if (val !== "" && val.length > 2) {
//         let speak2 = agentManager2.speak(
//             {
//                 type: "text",
//                 input: val,
//             }
...
//     let val = textArea2.value
//     if (val !== "") {
//         let chat2 = agentManager2.chat(val)
//         console.log(`Chat: ("${val}")`)
//         connectionLabel2.innerHTML = "Thinking.."
//         textArea2.value = ""
//     }
// }

async function speak2() {
    if (textArea2.value !== "" && textArea2.value.length > 2) {
        // Stop the other agent before starting
       
        await agentManager2.speak({ type: "text", input: textArea2.value });
        textArea2.value = "";
         if (agentManager) {
...
// console.log("Stream Type:", streamType)
//         }
        await agentManager2.chat(textArea2.value);
        connectionLabel2.innerHTML = "Thinking..";
        textArea2.value = "";
         if (agentManager) {
            await agentManager.disconnect();
...
// agentManager.rate() -> Rating the Agent's answers - for future Agents Analytics and Insights feature
function rate2(messageID, score) {
    let rate2 = agentManager2.rate(messageID, score)
    console.log(`Message ID: ${messageID} Rated:${score}\n`, "Result", rate2)
}

// agentManager.reconnect() -> Reconnect the Agent to a new WebRTC session
function reconnect2() {
    let reconnect2 = agentManager2.reconnect()
    console.log("Reconnect")
}
...
// (NOT IMPLEMENTED IN THIS DEMO)
// function disconnect() {
//     let disconnect = agentManager2.disconnect()
//     console.log("Disconnect")
// }

document.getElementById("stopButton2").addEventListener("click", () => stopAgent2());

 async function restartAgent2() {
    console.log("Connecting to Agent ID: ", agentId2)
await agentManager2.connect()

// Check for the set Stream type (Legacy/Fluent)
streamType2 = agentManager2.getStreamType()
console.log("Stream Type:", streamType2)
}

function stopAgent2() {
    if (agentManager2) {
        agentManager2.disconnect();
        console.log("Agent session stopped.");
        connectionLabel2.innerHTML = "Stopped";
        document.querySelector("#hidden_h2").innerHTML = `${agentManager2.agent.preview_name} Stopped`;
        restartAgent2()
        // document.querySelector("#hidden").style.display = "flex";
        // document.querySelector("#agent-right").style.display = "none";
        // actionButton2.setAttribute("disabled", true);
...
// Agent ID and Client Key check
if (agentId2 == "" || auth2.clientKey == "") {
    connectionLabel2.innerHTML = `<span style='color:red; font-weight:bold'> Missing agentID and auth.clientKey variables</span>`
    console.error("Missing agentID and auth.clientKey variables")
    console.log(`Missing agentID and auth.clientKey variables:\n\nFetch the data-client-key and the data-agent-id as explained on the Agents SDK Overview Page:\nhttps://docs.d-id.com/reference/agents-sdk-overview\n\nPaste these into their respective variables at the top of the main.js file and save.`)
}

// Chat/Speak Selection Logic
function switchModes2() {
    const options = document.querySelectorAll('#buttons input[name="option"]');
    const checkedIndex = Array.from(options).findIndex(opt => opt.checked);
    const nextIndex = (checkedIndex + 1) % options.length;
    options[nextIndex].checked = true;
...
// Event Listeners for Agent's built-in methods
actionButton2.addEventListener('click', handleAction2);
speechButton2.addEventListener('click', () => toggleStartStop2())
reconnectButton.addEventListener('click', () => reconnect2())
...
let agentManager2 = await createAgentManager(agentId2, { auth:auth2, callbacks:callbacks2, streamOptions:streamOptions2 });
console.log("Create Agent Manager: ", agentManager2)
